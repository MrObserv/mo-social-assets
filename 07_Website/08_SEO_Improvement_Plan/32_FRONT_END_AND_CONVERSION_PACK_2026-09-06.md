...
4. **Client naming.** The live `/advisory` page names Standard Chartered, Lloyds and TAMM Abu Dhabi. The build brief's house rules say a client is "a tier-1 UK bank". Those names are already published on your own page, so I have carried them into the prototype, but confirm that is intended rather than an inconsistency to fix.
